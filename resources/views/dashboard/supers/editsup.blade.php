    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <div class="relative p-4 shadow-md sm:rounded-lg">
                <h2 class="mb-5 text-2xl font-semibold text-center text-blue-500 font-inter">Edit Surat Perintah Lembur
                </h2>
                <div class="flex mb-5">
                    <div class="flex-auto">
                ...
                <form method="POST" action="/dashboard/super/{{ $supel->id }}" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="sup_nomor" id="sup_nomor"
                            ...
                            placeholder=" " value="{{ old('sup_nomor', $supel->sup_nomor) }}" required />
                        <label for="sup_nomor"
                        ...
                        <div class="relative z-0 w-full mb-6 group">
                            <div class="relative mb-3" data-te-datepicker-init data-te-input-wrapper-init>
                                <input type="text" name="sup_tanggal" id="sup_tanggal"
                                    ...
                                    placeholder="Tanggal"
                                    value="{{ old('sup_tanggal', date('d/m/Y', strtotime($supel->sup_tanggal))) }}" />
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                ...
                                <select id="sup_bulan" name="sup_bulan"
                                    ...
                                    @foreach ($bulan as $bln)
                                        {{-- <option value="{{ 2023 }}">{{ $bln }}</option> --}}
                                        @if (old('supel_bln', $supel->sup_bulan) == $no)
                                            <option value="{{ $no }}" selected>{{ $bln }}</option>
                                        @else
                    ...
                    <div>
                        ...
                        <trix-editor input="sup_keterangan">{!! old('sup_keterangan', $supel->sup_keterangan) !!}</trix-editor>
                    </div>
            </div>
